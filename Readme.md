...
WORKDIR /src
COPY ["AWSLambdaCSVToList.csproj", "AWSLambdaCSVToList/"]
RUN dotnet restore "AWSLambdaCSVToList/AWSLambdaCSVToList.csproj"

WORKDIR "/src/AWSLambdaCSVToList"
COPY . .
RUN dotnet build "AWSLambdaCSVToList.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AWSLambdaCSVToList.csproj" \
...
Execute unit tests
```
cd "AWSLambdaCSVToList/test/AWSLambdaCSVToList.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "AWSLambdaCSVToList/src/AWSLambdaCSVToList"
